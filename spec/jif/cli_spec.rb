...
describe Jif::CLI do
  describe '#search' do
    ...
    before do
      ...
      # Stub the no-results gif search request
      stub_request(:get, "http://api.giphy.com/v1/gifs/search?api_key=dc6zaTOxFJmzC&limit=1&q=a%20search%20that%20gives%20no%20results").
        with(:headers => {'Accept'=>'*/*', 'Accept-Encoding'=>'gzip;q=1.0,deflate;q=0.6,identity;q=0.3', 'User-Agent'=>'Faraday v0.9.2'}).
        to_return(:status => 200, :body => empty_response_body, :headers => {})
    ...
    it 'displays a 404 gif when there are no search results' do
      ...
      expect(Shellwords).to receive(:escape).and_call_original
      expect_any_instance_of(Jif::CLI).to receive(:system).with(include "#{Jif::CLI::IMGCAT_LOCATION}")
      subject.search(no_results_query_terms)
    end
